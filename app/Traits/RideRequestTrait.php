...
trait RideRequestTrait {
    ...
    public function acceptDeclinedRideRequest($ride_request,$request_data = null)
    {
        ...
        foreach ($nearbyDrivers as $key => $driver) {
            
            if( $nearby_driver != null )
            {
                $data['riderequest_in_driver_id'] = $driver->id;
        ...
        if (isset($notify_data)) {
            dispatch(new NotifyViaMqtt('new_ride_requested', json_encode($notify_data)));
        }
    ...
    public function completeRequest($ride_request) {
