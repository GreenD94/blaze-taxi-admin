...
class SettingController extends Controller
{
    ...
    public function layoutPage(Request $request)
    {
        ...
        switch ($page) {
            ...
            case 'mobile-config':
                ...
                $getSetting[] = 'RIDE_MODALITY_AUCTION';
                $getSetting[] = 'RIDE_MODALITY_EXPRESS';
            ...
            case 'payment-setting':
                ...
                break;
            case 'referrals-setting':
                $referrals_setting = [];
                $referrals_setting['referral_enabled'] = Setting::where('key','REFERRALS_ENABLED')->pluck('value')->first();
                $referrals_setting['referral_amount'] = Setting::where('key','REFERRAL_AMOUNT')->pluck('value')->first();

                $data  = view('setting.'.$page, compact('settings', 'page', 'referrals_setting'))->render();
                break;
            default:
    ...
    public function settingUpdate(Request $request)
    {
        ...
        Setting::updateOrCreate(['key'=> 'RIDE_MODALITY_AUCTION'], ['value' => $request->auction ?? 0, 'type' => 'ride']);
        Setting::updateOrCreate(['key'=> 'RIDE_MODALITY_EXPRESS'], ['value' => $request->express ?? 0, 'type' => 'ride']);
    ...
    public function referralSettingsUpdate(Request $request) {
        ...
        $input = [
            ...
            'value' => $request->referral_enabled,
        ];

        Setting::updateOrCreate(['key' => 'REFERRALS_ENABLED'],$input);
        
        $input = [
            'type'  => 'referrals',
            'key'   => 'REFERRAL_AMOUNT',
            'value' => $request->referral_amount,
        ];

        Setting::updateOrCreate(['key' => 'REFERRAL_AMOUNT'],$input);
