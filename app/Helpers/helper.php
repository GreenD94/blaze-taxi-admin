...
function saveRideHistory($data)
{
    ...
    switch ($data['history_type']) {
        ...
        case 'payment_status_message':
            ...
            $mqtt_event = 'ride_request_status';
            break;
        case 'drivers_offering':
            $data['history_message'] = __('message.ride.drivers_offering');
            $history_data = [
                'rider_id' => $ride_request->rider_id,
                // 'status' => optional($ride_request->payment)->payment_status,
                'rider_name' => optional($ride_request->rider)->display_name ?? '',
            ];
            $sendTo = removeValueFromArray(['driver','rider'], $user_type);
            $mqtt_event = 'drivers_offering';
            break;
        default:
            # code...
            break;
    ...
    if ($file) {
        ...
        if (is_array($file)) {
            foreach ($file as $key => $value) {
                $mediaItem = $model->addMedia($value)->toMediaCollection($name);
            ...
            $mediaItem = $model->addMedia($file)->toMediaCollection($name);
            $filePath = $mediaItem->getPath();
            $extension = pathinfo($mediaItem->file_name, PATHINFO_EXTENSION);
            if (in_array(strtolower($extension), $imageExtensions)) {
                ImageOptimizer::optimize($filePath);
...
function getNearbyDrivers(RideRequest $riderequest)
{
    $radius = Setting::where('type', 'DISTANCE')->where('key', 'DISTANCE_RADIUS')->pluck('value')->first() ?? 50;
    $latitude = $riderequest->start_latitude;
    $longitude = $riderequest->start_longitude;
    $unit = $riderequest->distance_unit ?? 'km';
    ...
    ->where('user_type', 'driver')->where('status', 'active')
        // ->where('is_online', 1)->where('is_available', 1)
    ...
    $nearbyDrivers = $nearby_driver->get();

    return $nearbyDrivers;
}

function generateRefererCode($user)
{
...
// deposit cash difference to rider wallet
function depositCashDifferenceToRiderWallet(RideRequest $rideRequest) {

    $user = $rideRequest->rider;
    $wallet = Wallet::firstOrCreate(['user_id' => $user->id]);

    $customerBalance =  $wallet->total_amount ?? 0; // 90
    $outstanding = max(0, $rideRequest->total_amount - $customerBalance); // 112 - 90 = 22

    $cashDifference = $rideRequest->cash_collected - $outstanding; // 125 - 22 = 103

    $wallet->currency = SettingData('CURRENCY', 'CURRENCY_CODE') ?? 'USD';
    $wallet->total_amount = $customerBalance + $cashDifference; // 90 + 103 = 193
    ...
    try {
        ...
        WalletHistory::create([
            ...
            'user_id'           => $user->id,
            'type'              => 'credit',
            'transaction_type'  => 'cash difference',
 ...
 * @param  mixed $amount
 * @param  mixed $type
 * The type of transaction. Valid values are 'credit' or 'debit'.
 * @return void
 */
function singleWalletTransaction($userId, $amount, $type) {
...
// check for bonus
function checkBonusesForDriver(User $driver) {
    ...
    foreach ($bonuses as $bonus) {
        ...
        if ($bonus->start_date_type == 'fixed') {
            $startDate = $bonus->starts_at;
        } else if ($bonus->start_date_type == 'verification_date') {
            $startDate = $driver->driverDocument()
        ...
        if ($bonus->end_date_type == 'fixed') {
            $endDate = $bonus->ends_at;
        } else if ($bonus->end_date_type == 'days_to_expiration') {
            $endDate = new Carbon\Carbon($startDate);
            $endDate->addDays($bonus->days_to_expiration)->endOfDay();
        }

        $ridesQty = $bonus->rides_qty;
        $ridesCompletedQty = $driver->driverRideRequestDetail()
            ->whereBetween('created_at', [$startDate, $endDate])
            ->where('status', 'completed')
            ->count();

        //     $bonus['qty'] = $ridesCompletedQty;

        $bonus_data[] = [
            'name' => $bonus->name,
            'qty' => $ridesCompletedQty,
            $startDate,
            $endDate
        ];
        ...
        if ($startDate && $endDate) {
            if ($now->between($startDate, $endDate)) {
                $in_force_bonuses[] = $bonus;   
                continue;
            }
        }
    }

    // check for in force bonuses
    foreach ($in_force_bonuses as $bonus) {
        $ridesQty = $bonus->rides_qty;
        $ridesCompletedQty = $driver->driverRideRequestDetail()
            ->whereBetween('created_at', [$startDate, $endDate])
            ->where('status', 'completed')
        ...
        if ( $ridesCompletedQty >= $ridesQty) {

            if (!$driver->bonuses()->where('bonus_id', $bonus->id)->exists()) {

                $driver->bonuses()->attach($bonus, ['amount' => $bonus->amount, 'status' => 'claimed']);

                $wallet =  Wallet::firstOrCreate(
                    [ 'user_id' => $driver->id ]
                );

                $total_amount = $wallet->total_amount + $bonus->amount;
            ...
            // $driver->bonus_amount += $bonus->amount;
        }

    }

    return collect($in_force_bonuses)->sortByDesc('qty')->values();


}

// get bcv usd rate
function getBcvUsdRate() {

    if (Cache::has('bcv_usd_rate')) {
        return Cache::get('bcv_usd_rate');
    } else {
        $url = 'https://pydolarve.org/api/v1/dollar?page=bcv&monitor=usd';
        $response = Http::get($url);

        if ($response->failed()) {
            return env('BCV_USD_RATE', 0);
        }

        $response = json_decode($response->body());
        Cache::put('bcv_usd_rate', $response->price, $seconds = 60 * 24);
